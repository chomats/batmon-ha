...
from bmslib.serialbattery.jkserialio import JKSerialIO, s_decode_O1, decode_info
...
async def main():
    ...
    for dev in user_config.get('devices', []):
        ...
        if dev.get('is_serial', False):
            continue
        device = construct_bms(dev, verbose_log, devices)
    ...
    for device in bms_list:
        device.set_keep_alive(user_config.get('keep_alive', False))

        if isinstance(device, VirtualGroupBms):
            group_bms = device
            for member_ref in device.get_member_refs():
                if member_ref not in bms_by_name:
                    logger.warning('Please choose one of these names: %s', set(bms_by_name.keys()))
                    raise Exception("unknown device '%s' in group %s" % (member_ref, group_bms))

                member_name = bms_by_name[member_ref].name
                if member_name in groups_by_bms:
                    raise Exception("can't add device %s to multiple groups %s %s", member_name,
                                    groups_by_bms[member_name], group_bms)
                
                groups_by_bms[member_name] = group_bms.group
                device.add_member(bms_by_name[member_ref])
    ...
    for device in user_config.get('devices', []):
        name = device['name']
        if not device.get('is_serial', False):
            continue
        assert name not in names, "duplicate name %s" % name
        names.add(name)

        bms_bt = construct_bms(device, verbose_log, devices)
        
        bms_s = BmsSampler(
            bms_bt=bms_bt,
            name=name, address=int(device['address_serial']), 
